...
void igabeam(double Lx, double Ly, double Lz, int mref, int porder, int bc_periodic_x, int bc_periodic_y, int bc_periodic_z, int **bc_periodic_, int **nelem_global_, double ***knotVector_global_, double **wVector_global_, double **XVector_global_)
{
    ...
    *XVector_global_    = (double*)malloc(ndim*nbasis_global[0]*nbasis_global[1]*nbasis_global[2]*sizeof(double));
    ...
    for (int k=0; k<nbasis_global[2]; k++) {
        ...
        XVector_global[ndim*ia+0]=((double)i-0.5)/((double)(nbasis_global[0]-2))*L[0];
        XVector_global[ndim*ia+1]=((double)j-0.5)/((double)(nbasis_global[1]-2))*L[1];
        XVector_global[ndim*ia+2]=((double)k-0.5)/((double)(nbasis_global[2]-2))*L[2];
        ia++;
    }}}
}
